# Portfolio
👨‍💻 Siva Sanjeev Kumar – Cybersecurity Portfolio

Welcome to my personal cybersecurity portfolio website.
I am passionate about cybersecurity, ethical hacking, penetration testing, and securing digital systems against modern threats.

🚀 About Me

Hi, I'm Siva Sanjeev Kumar
Cybersecurity Enthusiast | Ethical Hacker | Penetration Tester

I am focused on:

Web Application Security
Vulnerability Assessment
Penetration Testing
Network Security
Linux & Windows Security
SOC & Threat Monitoring
Bug Hunting

I continuously learn advanced cybersecurity techniques and work on real-world security projects to improve my skills.

🛠️ Technologies Used
HTML5
CSS3
JavaScript
Git & GitHub
Linux
Kali Linux
Wireshark
Nmap
Burp Suite
📂 Portfolio Features

✔️ Responsive Modern Design
✔️ Cybersecurity Themed UI
✔️ About Me Section
✔️ Skills Showcase
✔️ Certifications Section
✔️ Projects Section
✔️ Contact Information
✔️ Social Media Links

📌 Projects
🔐 Vulnerability Scanner

A basic security scanner for detecting common vulnerabilities.

🌐 Network Analysis Lab

Traffic monitoring and packet analysis using Wireshark.

🛡️ Web Security Testing

Testing web applications for vulnerabilities like XSS, SQL Injection, and Authentication flaws.

📜 Certifications
Google Cybersecurity Certificate (Learning)
Ethical Hacking & Cybersecurity Training
Networking Fundamentals
Linux Fundamentals
